...
void tearDown(void) { clear_roster(); }

static void populate_roster(roster_t *expected) {
  for (size_t i = 0; i < expected->count; ++i)
    TEST_ASSERT_TRUE(
        add_student(expected->students[i].name, expected->students[i].grade));
}
...
static void test_adding_student_adds_to_roster(void) {
  ...
  roster_t input = {.count = 1,
                    .students = {(student_t){.grade = 2, .name = "Aimee"}}};
...
static void test_adding_students_to_different_grades_adds_to_same_roster(void) {
  TEST_IGNORE();
  roster_t input = {2, {(student_t){3, "Chelsea"}, (student_t){7, "Logan"}}};

  roster_t expected = {2, {(student_t){3, "Chelsea"}, (student_t){7, "Logan"}}};
...
static void test_students_in_same_grade_are_sorted_in_same_roster(void) {
  TEST_IGNORE();
  roster_t input = {3,
                    {(student_t){1, "Peter"}, (student_t){1, "Anna"},
                     (student_t){1, "Barb"}}};
...
static void test_students_with_grades_listed_in_same_sorted_roster(void) {
  TEST_IGNORE();
  roster_t input = {7,
                    {(student_t){2, "Peter"}, (student_t){1, "Anna"},
                     (student_t){1, "Barb"}, (student_t){2, "Zoe"},
                     (student_t){2, "Alex"}, (student_t){3, "Jim"},
                     (student_t){1, "Charlie"}}};

  roster_t expected = {7,
                       {(student_t){1, "Anna"}, (student_t){1, "Barb"},
                        (student_t){1, "Charlie"}, (student_t){2, "Alex"},
                        (student_t){2, "Peter"}, (student_t){2, "Zoe"},
                        (student_t){3, "Jim"}}};
...
static void
test_grade_lists_students_in_that_grade_in_alphabetical_order(void) {
  TEST_IGNORE();
  roster_t input = {3,
                    {(student_t){5, "Franklin"}, (student_t){5, "Bradley"},
                     (student_t){1, "Jeff"}}};
